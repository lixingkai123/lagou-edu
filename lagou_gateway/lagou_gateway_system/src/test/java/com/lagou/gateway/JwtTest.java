...
public class JwtTest {
    ...
    public void test5() {
        ...
        Date expirationDate = new Date(time);
        //构建一个jwtBuilder对象
        JwtBuilder jwtBuilder = Jwts.builder().setId("9527").setSubject("lagou_shop").setIssuedAt(new Date()).setExpiration(expirationDate).claim("role","admin").signWith(SignatureAlgorithm.HS256, "lagou");
        //创建
        String token = jwtBuilder.compact();
        System.out.println(token);
        System.out.println("---------------------------");
